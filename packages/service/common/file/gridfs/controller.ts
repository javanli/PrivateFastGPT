...
export function delFileByFileIdList({
  bucketName,
  fileIdList,
  retry = 3
}: {
  ...
  try {
    ...
    if (retry > 0) {
      delFileByFileIdList({ bucketName, fileIdList, retry: retry - 1 });
    }
  }
}
